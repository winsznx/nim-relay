...
**Carried forward.** (1) The `service_role` key is still outstanding — the Worker's `SupabaseAuthStore` needs it and `getAuthStore()` falls back to the in-memory store until it is set. (2) Every `public` table has RLS enabled with **no policies** (Supabase auto-enables RLS on new tables); only `service_role` can touch data. Explicit RLS policy design is a PRD Phase 9 item and a hard prerequisite for any direct browser→Supabase access.
