...
const LEFT: readonly Destination[] = [
  { label: 'World', to: '/', icon: 'world', matches: ['world', 'relay', 'chronicle', 'proof', 'proofRelay', 'station'] },
  { label: 'Inbox', to: '/inbox', icon: 'inbox', matches: ['inbox', 'invite'] },
]
const RIGHT: readonly Destination[] = [
  { label: 'Crew', to: '/crew', icon: 'crew', matches: ['crew', 'rivals'] },
  { label: 'Profile', to: '/profile', icon: 'profile', matches: ['profile', 'runner', 'privacy'] },
]
...
function NavLink({ destination, current, badge }: { destination: Destination; current: RouteMatch['name']; badge?: number }) {
  ...
  return (
    <a className="nr-nav__item" aria-current={active ? 'page' : undefined} aria-label={badge ? `${destination.label}, ${badge} unread` : undefined} {...linkProps(destination.to)}>
...
export function BottomNav({ current, unread }: { current: RouteMatch['name']; unread: number }) {
  return (
    <nav className="nr-nav" aria-label="Main">
      ...
      <button type="button" className="nr-nav__play" onClick={() => openOverlay(PLAY_OVERLAY)} aria-haspopup="dialog">
        <span className="nr-nav__play-mark" aria-hidden="true">
