...
/** Shoulder-relative hand targets. The right hand carries the baton. */
const BATON_HAND = new THREE.Vector3(-0.1, -0.2, 0.33)
const BATON_HAND_TUCK = new THREE.Vector3(-0.1, -0.16, 0.34)
...
const HANDS = [
  { ride: BALANCE_HAND, tucked: BALANCE_HAND_TUCK, elbow: BALANCE_ELBOW_POLE, balance: 1 },
  { ride: BATON_HAND, tucked: BATON_HAND_TUCK, elbow: BATON_ELBOW_POLE, balance: 0 },
] as const
...
export class CourierStance {
  ...
  private advance(drive: CourierDrive): void {
    ...
    const airborne = riding && drive.airborne && !drive.sliding
    this.tuck = damp(this.tuck, airborne ? 1 : 0, airborne ? 9 : 18, dt)
    this.lean = damp(this.lean, riding ? THREE.MathUtils.clamp(drive.lateralVelocity / 7, -1, 1) : 0, 7, dt)
  ...
  private solve(time: number): void {
    ...
    const roll = this.lean * 0.3 + Math.sin(time * 17) * this.wobble * 0.1
    const crouch = 0.29 + this.flow * 0.04 + this.compress * 0.12 + this.rail * 0.03 - this.tuck * 0.06 + sway * 0.006
    const pitch = 0.55 + this.flow * 0.1 + this.compress * 0.18 + this.tuck * 0.12 - this.flinch * 0.5
    ...
    for (let index = 0; index < 2; index++) {
      ...
      this.orient(leg.end, this.boardRoll, FOOT_YAW, 0)
    }
    ...
    for (let index = 0; index < 2; index++) {
      ...
      const { ride, tucked, elbow, balance } = HANDS[index]!
      this.chain(arm.upper, index === 0 ? j.clavicle_l : j.clavicle_r, roll, chestYaw, pitch)
      this.v1.copy(ride).lerp(tucked, this.tuck)
      this.v1.x += this.rail * 0.06 * balance
      this.v1.y += flinchLift + sway * 0.012
      this.target.copy(this.v1).applyQuaternion(this.chest).add(arm.upper.position)
      this.solveLimb(arm, this.target, this.pole.copy(elbow).applyQuaternion(this.chest))
    }
