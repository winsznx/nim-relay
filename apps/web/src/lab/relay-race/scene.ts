...
function courierMesh(color: number, ghost = false): THREE.Group {
  ...
  const torso = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.78, 0.34), mat(ghost ? GHOST_C : 0x2b3a55))
  torso.position.y = 0.85
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.24, 12, 10), mat(ghost ? GHOST_C : 0xdfe6f2))
