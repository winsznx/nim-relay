...
3. `confirmHandoff` looks the transaction up by hash via plain JSON-RPC and only accepts it after `verifyHandoffTransaction` (`packages/relay-protocol/src/verify-transaction.ts`) checks, independently of anything the client claims: recipient, value, network, that the payer isn't the recipient (no self-pass), that the transaction's data field decodes to the exact commitment from step 1, that the transaction executed rather than reverted, and that it has at least `MIN_CONFIRMATIONS` confirmations. A handoff is never canonical on a client's say-so.
...
The paying address is deliberately not required to match the address the holder signed in with. Nimiq Pay sweeps incoming NIM from a player's receiving address into an internal address and pays from there, so the two never match for real Nimiq Pay users (found on mainnet, 2026-09-18). The commitment in the data field is issued only to the holder's session and binds this one pass, so a transfer that carries it to the chosen runner is the holder's pass. The worst an outsider holding a leaked commitment could do is pay 1 NIM of their own to move the baton exactly where the holder already chose.
